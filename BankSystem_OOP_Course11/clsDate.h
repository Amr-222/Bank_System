...
public:
	...
	static string GetTimeNow(string deli=":") {
		time_t t = time(0);
		tm* t_now = localtime(&t);

		return to_string(t_now->tm_hour) + deli + to_string(t_now->tm_min) + deli + to_string(t_now->tm_sec);

	}

	static string GetDateTimeStringNow(string sep=" - ") {

		return DateToString(clsDate()) + sep + GetTimeNow();
